// ============================================
// BIT SOFTWARE — Admin Email Orders
// ============================================
...
import { getAllEmailOrders, updateEmailOrder } from '@/api/emailOrderApi';
...
export default function AdminEmailOrders() {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await getAllEmailOrders({
        page,
        limit: 20,
  ...
  const patchStatus = async (id, field, value) => {
    try {
      await updateEmailOrder(id, { [field]: value });
  ...
  return (
    <>
      <SEOHead title="Email Orders — Admin" />
      <div style={{ padding: 'clamp(1rem, 3vw, 1.5rem)' }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
          <div>
            <h1 className="h4" style={{ margin: 0, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <ShoppingCart size={22} /> Email Orders
            </h1>
        ...
        <div className="card-elevated" style={{ overflowX: 'auto' }}>
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: '3rem', color: 'var(--color-text-muted)' }}>No Email orders yet.</div>
          ) : (
