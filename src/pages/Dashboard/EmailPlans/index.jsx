// ============================================
// BIT SOFTWARE — Admin Email Plans (catalog)
// ============================================
...
import {
  getAllEmailPlans, createEmailPlan, updateEmailPlan, deleteEmailPlan,
} from '@/api/emailPlanApi';
...
function PlanModal({ initial, onClose, onSaved }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEdit) {
        await updateEmailPlan(initial._id, payload);
        ...
        <form onSubmit={handleSubmit}>
          <div className="domains__form-grid">
            <div className="domains__field">
              <label>Slug *</label>
              <input className="input" value={form.slug} onChange={(e) => set('slug', e.target.value)} placeholder="email-starter" required disabled={isEdit} />
            </div>
            <div className="domains__field">
              <label>Name *</label>
...
export default function AdminEmailPlans() {
  ...
  const fetchPlans = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await getAllEmailPlans();
  ...
  return (
    <>
      <SEOHead title="Email Plans — Admin" />
      <div style={{ padding: 'clamp(1rem, 3vw, 1.5rem)' }}>
        ...
        <div className="card-elevated" style={{ overflowX: 'auto' }}>
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: '3rem', color: 'var(--color-text-muted)' }}>No email plans yet. Add one to get started.</div>
          ) : (
