...
import {
  ...
  LogOut, Menu, X, Home, ChevronRight, MapPin, Package, Mail,
} from 'lucide-react';
...
const NAV_ITEMS = [
  ...
  { id: 'email', label: 'My Email', icon: Mail, to: '/my-account?tab=email' },
  { id: 'gmb', label: 'My GMB', icon: MapPin, to: '/my-account?tab=gmb' },
  { id: 'services', label: 'Services', icon: Package, to: '/my-account?tab=services' },
  { id: 'wallet', label: 'Account Balance', icon: Wallet, to: '/my-account?tab=wallet' },
...
function resolveActiveSection(pathname, searchParams) {
  ...
  if (pathname.startsWith('/my-account/email')) return 'email';
...
function resolvePageTitle(pathname, section) {
  ...
  if (/\/my-account\/email\/[^/]+/.test(pathname)) return 'Email Details';
